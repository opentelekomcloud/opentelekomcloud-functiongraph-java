 ...
 * - ECS_ENDPOINT_URL ecs endpoint url, default:  "https://ecs.eu-de.otc.t-systems.com"
 * - STOP_TYPE stop type "HARD" or "SOFT", default: "SOFT"
 * 
 * see: https://docs.otc.t-systems.com/elastic-cloud-server/api-ref/apis_recommended/batch_operations/stopping_ecss_in_a_batch.html
 */
...
/**
 * Stop ECS Instance(s) example.
 */
public class StopECSInstance {

  private static final String DEFAULT_ECS_ENDPOINT_URL = "https://ecs.eu-de.otc.t-systems.com";
  private static final String DEFAULT_STOP_TYPE = "SOFT";
  ...
  public String handleRequest(final JsonObject event, final Context context) {
    ...
    String stopType = context.getUserData("STOP_TYPE");
    if (stopType == null || stopType.trim().isEmpty()) {
      log.warn(String.format("Environment variable STOP_TYPE not set. Using default: %s", DEFAULT_STOP_TYPE));
      stopType = DEFAULT_STOP_TYPE;
    }
    if (!stopType.equals("HARD") && !stopType.equals("SOFT")) {
      log.error("Incorrect value for STOP_TYPE, allowed SOFT or HARD");
      return "Incorrect value for STOP_TYPE, allowed SOFT or HARD";
    }
    ...
    try {
      ...
      String body = String.format("{\"os-stop\":{\"servers\":[%s],\"type\": \"%s\"}}", getServerIds(instanceId),
          stopType);

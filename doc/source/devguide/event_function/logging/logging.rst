...
To output logs from your function code, you can use methods on
``java.lang.System``, or any logging module that writes to ``stdout``
or ``stderr``.
...
The ``opentelekomcloud-functiongraph-java`` library provides a logger class
named ``RuntimeLogger`` that can be accessed from the context object.
...
The Java runtime logs the **Start invoke request** and
**Finish invoke request** lines for each invocation.
...
* **RequestId** - The unique request Id for the invocation
* **duration** - The amount of time that your function's handler
  method spent processing the event.
...
The Api implementation of **Context**, currently does not provide a
method to get the current log level the RuntimeLogger is using.
